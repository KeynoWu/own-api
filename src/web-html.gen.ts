...
// DR-14（auto 报错归因可见）：候选链每一跳的真实模型/渠道/状态早已记在 chainAttempts 里，展示层必须消费——
// 计费模型列挂「⚠ 候选名 状态」失败痕迹；入口被剔除的候选（如 key 全冷却）挂「⊘ 候选名」；行悬停拼候选明细（哪个模型）+ retries 明细（哪把 key）。
// 两层都要：失败链常见形态是前一跳 4xx 进了 chainAttempts、后续跳 key 冷却进了 chainExcluded——漏掉后者就只剩光秃秃的 auto 名
const failedChain = (l) => (l.chainAttempts || []).filter((a) => a.status >= 400);
const chainTip = (l) => [...(l.chainAttempts || []).map((a) => \`候选 \${a.name}（\${a.channel}）→ \${a.status}\${a.error ? '：' + a.error.slice(0, 200) : ''}\`), ...(l.chainExcluded || []).map((e) => \`剔除 \${e.name}（\${e.kind === 'hard' ? '硬约束' : '瞬态'}）：\${e.reason}\`)].join('\\n');
const rowTip = (l) => [chainTip(l), (l.retries || []).join('\\n'), l.error || ''].filter(Boolean).join('\\n');
const modelBits = (l) => {
  const s = el('span', { class: 'mono' });
  s.append(l.requestedModel || '-');
  if (l.routedTo && l.routedTo !== l.requestedModel) s.append(' → ' + l.routedTo);
  for (const a of failedChain(l))
    s.append(el('span', { class: 'mono', style: 'color:var(--err);margin-left:8px;font-size:11px', title: (a.error || '').slice(0, 300) }, '⚠ ' + a.name + ' ' + a.status));
  for (const e of l.chainExcluded || [])
    s.append(el('span', { class: 'mono muted', style: 'margin-left:8px;font-size:11px', title: e.reason }, '⊘ ' + e.name));
  return s;
};
...
const ovSt = { hours: 'day', model: '', fstat: '', tab: 'logs', refresh: 30 }; // hours: 'day'=自然日（本地零点起）；数字=滚动小时窗
{
  ...
  views.overview = async () => {
    ...
    if (ovSt.tab === 'logs') {
      ...
      // DR-14：模型统计按真实候选归因——成功=落点候选（publicName），失败链=最后尝试候选；未及尝试（404 无候选等）回落 auto 名
      const g = ovSt.tab === 'provider' ? groupAgg((l) => l.channelName) : groupAgg((l) => l.publicName || l.requestedModel);
      const t = el('table');
...
views.models = async () => {
  ...
  const editAuto = (a) => form(a ? '编辑 ' + a.publicName : '新增自动路由', [
    ...
    { name: 'firstHop', label: '首跳策略（DR-16）', type: 'select', value: a?.firstHop ?? 'random', options: [
      { value: 'random', label: '加权随机（默认）——按有效权重分流，摊开流量、候选保持温热' },
      { value: 'best', label: '当前最强——确定性选有效权重最高者，不摇号' },
    ], hint: 'best：新会话/粘性过期时直奔「当前最强」（weight×健康×速度，动态）。粘性、变慢绕行、报错换链语义全部不变——适合“一直用好用的，除非它出状况”' },
    ...
    const firstHop = v.firstHop === 'best' ? 'best' : 'random';
    const body = a ? { publicName: v.publicName, candidates, stickyTtlMs: v.stickyTtlMs === '' ? 300000 : Number(v.stickyTtlMs), firstHop, note: v.note || '' }
      : { type: 'auto', publicName: v.publicName, candidates, stickyTtlMs: v.stickyTtlMs === '' ? 300000 : Number(v.stickyTtlMs), firstHop, note: v.note || '' };
  ...
  for (const r of routes) {
    ...
    t.append(el('tr', {},
      ...
      el('td', { class: 'mono muted' }, r.type === 'auto' ? (r.stickyTtlMs ? Math.round(r.stickyTtlMs / 1000) + 's' : '关') + (r.firstHop === 'best' ? ' ·最强首跳' : '') : '-'),
...
views.logs = async () => {
  ...
  let onlyErr = false;
  const render = (l) =>
    el('div', { class: 'log-line', style: l.ok ? '' : 'background:rgba(248,81,73,.06)', title: rowTip(l) },
  ...
  box.append(el('div', { class: 'toolbar' },
    ...
    el('span', { class: 'muted' }, '×N 表示该请求在号池里试过 N 个 key；⚠=auto 链内失败候选（名+状态）⊘=入口即被剔除的候选，悬停看细节'),
  ), list);
  return box;
};
