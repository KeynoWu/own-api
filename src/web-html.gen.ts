...
function form(title, fields, onSubmit) {
  ...
  const submit = async () => {
    ...
    try { await onSubmit(vals); if (dlg.__form === myForm) dlg.close(); } catch (e) { toast(e.message, true); }
  };
...
const ovSt = { hours: 24, model: '', fstat: '', tab: 'logs', refresh: 30 };
{
  ...
  views.overview = async () => {
    ...
    if (o.logRetention && allLogs.length >= o.logRetention) {
      box.append(el('div', { class: 'card muted', style: 'margin-top:8px;font-size:12px' }, '日志仅保留最近 ' + o.logRetention + ' 条且已达上限——更早的请求记录已被丢弃，本窗口统计可能不完整'));
    }
