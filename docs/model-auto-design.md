...
- **迟滞带（修正 N1）**：
  ...
  - **手动清空（v2.5）**：管理端 `DELETE /api/routes/:id/sticky` 清空该路由全部绑定；路由页每行「粘性立即生效」按钮即走此口——调完权重/候选想马上接管分流时用（此前只能等 TTL 过期或重启网关）。改名与删除 auto 路由时服务端同步按旧名清粘性，不留死条目；`/auto-health?route=` 可按路由名探测绑定数。
