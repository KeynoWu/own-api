...
| 管理台 | 渠道与号池、模型路由（单模型/自动路由同页管理，新增时选类型；含候选健康分/粘性运行时视图）、对外 key、使用统计首页（趋势/成本/缓存命中）、实时日志、接入代码片段，全部可视化操作 |
...
```bash
npm test         # 211 项：对"理想上游"的功能面（鉴权/同协议/跨协议/流式/号池/用量/边界 + auto 全套：
                # 硬过滤/粘性迟滞/续链与终态/ACL/链预算/断开止损/key 泄漏哨兵 + 速度排行口径与 auto 链败归因 + 配置组导出/dryRun 等价/幂等/双实例往返
                # + 审查修复轮 §15：auto×流式两段式提交三态、限额预占与并发拒绝、N11 不覆写、估算器口径、import 回执）
npm run test:hard # 141 项：对"脏上游"的加固回归 + 前端 DOM 桩（CRLF 分帧、JSON 冒充流式、卡死、离谱 Retry-After、断开 499、
